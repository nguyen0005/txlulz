...
async def leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
  ...
  message = "Top Bảng Xếp Hạng Free Fire\n\n"
...
async def trutien(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
  if not is_user_admin(update.effective_user.id):
      await update.message.reply_text('Bạn không có quyền sử dụng lệnh này ⚠️')
      return

  args = context.args
  if len(args) != 2 or not args[1].isdigit():
      await update.message.reply_text('⚠️ Cú pháp không đúng\n>> VD: /trutien @username số_tiền')
      return
  ...
  with open('data.json', 'r+', encoding='utf-8') as file:
      ...
      if user:
          ...
          await update.message.reply_text('Không tìm thấy người dùng ⚠️')

async def money(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
  args = context.args
  if len(args) != 1:
      await update.message.reply_text('⚠️ lệnh không hợp lệ\n>> VD: /money @username')
      return
  ...
  try:
      with open('data.json', 'r+', encoding='utf-8') as file:
          ...
          if str(user_id) in users:
              ...
              await update.message.reply_text("Tài khoản không tồn tại ⚠️")
  except FileNotFoundError:
      await update.message.reply_text("Không tìm thấy dữ liệu ⚠️")
...
async def listcode(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
  ...
  try:
      ...
      if not codes:
          await update.message.reply_text("⚠️ Hiện không có giftcode nào")
          return
      ...
      await update.message.reply_text("⚠️ Không tìm thấy dữ liệu giftcode")

async def chuyentien(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    args = context.args
    if len(args) != 2 or not args[1].isdigit():
        await update.message.reply_text('⚠️ Cú pháp không hợp lệ\n>> Sử dụng: /chuyentien @username số_tiền')
        return
    ...
    try:
        with open('data.json', 'r+', encoding='utf-8') as file:
            ...
            if not sender:
                await update.message.reply_text('⚠️Không tìm thấy thông tin của bạn\n>> Hãy Đảm bảo bạn đã đăng ký')
                return

            if not receiver:
                await update.message.reply_text('⚠️ Không tìm thấy người nhận')
                return

            if sender['balance'] < amount:
                await update.message.reply_text('⚠️ Bạn không có đủ tiền để thực hiện giao dịch này')
                return
...
async def taocode(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
  ...
  if len(args) != 3 or not args[1].isdigit() or not args[2].isdigit():
      await update.message.reply_text('⚠️Cú pháp không đúng\n>> VD: /taocode abc [số_tiền] [số_lượt_dùng]')
      return
  ...
  with open('giftcode.json', 'r+', encoding='utf-8') as file:
      ...
      if code in giftcodes:
          await update.message.reply_text('⚠️ Giftcode này đã tồn tại')
      else:
...
async def giftcode(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
  ...
  if not context.args:
      await update.message.reply_text('>> Vui lòng nhập giftcode\nVí dụ: /giftcode GIFT100')
      return
      ...
      if gift_code in giftcodes and giftcodes[gift_code]['uses'] > 0:
          ...
          with open('data.json', 'r+', encoding='utf-8') as user_file:
              ...
              if user:
                  ...
                  await update.message.reply_text('⚠️ Bạn chưa được đăng ký\n>> sử dụng lệnh /register')
                  return
          ...
          await update.message.reply_text('⚠️ Giftcode không hợp lệ hoặc đã hết lượt sử dụng')
