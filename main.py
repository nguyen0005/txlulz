...
YOUR_TOKEN = '6873640340:AAG_s4yODod8dp51dblSLIeUdtkcqPYZuSA'
admin_ids = [] 
...
from message import consolations
...
async def bowling(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
  ...
  if len(args) != 2 or not args[1].isdigit():
      await update.message.reply_text('⚠️Sử dụng lệnh sai\n>> VD: /bowling [bc/bl/bt/bx] [tiền cược]')
      return
  ...
  if not user_info:
    await update.message.reply_text('⚠️ Bạn chưa được đăng ký>> Hãy sử dụng lệnh /register')
    return
...
async def darts(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
  ...
  user_id = update.effective_user.id
  if len(args) != 2 or not args[1].isdigit():
      await update.message.reply_text('⚠️ Sử dụng lệnh sai\n>> VD: /darts [t/d] [tiền cược]')
      return
  ...
  if not user_info:
      await update.message.reply_text('⚠️Bạn chưa được đăng ký\n>> Hãy sử dụng lệnh /register')
      return
...
async def slot(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
  ...
  if not context.args:
      await update.message.reply_text("⚠️Cú pháp không hợp lệ\n>> Sử dụng: /S [tiền cược]")
      return

  try:
      bet_money = int(context.args[0])  
  except ValueError:
      await update.message.reply_text("⚠️ Tiền cược phải là một số. Vui lòng nhập lại")
      return
...
async def taixiu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    args = context.args
    user_id = update.effective_user.id
    if len(args) != 2 or (not args[1].isdigit() and args[1].lower() != "all"):
        await update.message.reply_text('⚠️ Hãy nhập lại theo mẫu\n>> /taixiu [tài/xỉu/tl/tc/xl/xc] [tiền cược]')
        return
    ...
    with open("data.json", "r+", encoding="utf-8") as file:
        ...
        if user is None:
            await update.message.reply_text('⚠️ Thông tin của bạn chưa được đăng ký\n>> Hãy sử dụng lệnh /register')
            return

        # Kiểm tra nếu người dùng đặt cược "all"
        if args[1].lower() == "all":
            bet_money = user['balance']
        else:
            bet_money = int(args[1])
        ...
        if (choice == result) or (choice == result_combined):
            ...
            # Gửi lời an ủi ngẫu nhiên từ message.py khi người dùng thua
            consolation_message = random.choice(consolations)
            await update.message.reply_text(f"{consolation_message}")
...
async def chanle(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
  ...
  if "chan" in choice_input or "chẵn" in choice_input:
    ...
    await update.message.reply_text('⚠️ Lựa chọn không hợp lệ\n\n>> Hãy chọn Chẵn hoặc Lẻ')
    return
  ...
  with open('data.json', 'r+', encoding='utf-8') as file:
      ...
      if user and money >= chanle_min_bet:
          ...
          if user['balance'] < money:
              remaining_balance = format_currency(user['balance'])  
              await update.message.reply_text(f'⚠️ Bạn chỉ còn {remaining_balance}, không thể cược số tiền lớn hơn số dư của bạn ‼️',parse_mode='HTML')
              return
          ...
          if not user:
              await update.message.reply_text('⚠️ Bạn cần đăng ký trước khi chơi\n >> Hãy Sử dụng lệnh /register')
          elif money < 5000:
              await update.message.reply_text(f'⚠️ Mức cược tối thiểu là {chanle_min_bet:,} VNĐ ‼️')

async def doanso(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
  args = context.args
  if len(args) != 2 or not args[0].isdigit() or not args[1].isdigit():
      await update.message.reply_text('⚠️ Sử dụng lệnh sai\n>> VD: /doanso (số bạn đoán từ 1 đến 6) (số tiền)')
      return
  ...
  if guess < 1 or guess > 6:
      await update.message.reply_text('⚠️ Số bạn đoán phải từ 1 đến 6')
      return
  ...
  with open('data.json', 'r+', encoding='utf-8') as file:
      ...
      if user and money >= doanso_min_bet:
          ...
          if user['balance'] < money:
              remaining_balance = format_currency(user['balance'])  
              await update.message.reply_text(f'⚠️ Bạn chỉ còn {remaining_balance}\n❌ không thể cược số tiền lớn hơn số dư của bạn',parse_mode='HTML')
              return
          ...
          if not user:
              await update.message.reply_text('⚠️ Bạn cần đăng ký trước khi chơi\n>> Sử dụng lệnh /register')
          elif money < 10000:
              await update.message.reply_text(f'⚠️ Mức cược tối thiểu là <b>{doanso_min_bet:,} VNĐ</b> ‼️',parse_mode='HTML')
...
async def mophien(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
  ...
  if args and args[0].lower() == "auto":
      ...
      await update.message.reply_text('⚠️ Sai cú pháp! Dùng /mophien auto để bắt đầu và /mophien stop để dừng')
...
async def bet(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
  ...
  if session and session["is_recording"]:
      ...
      if len(bet_message) != 2 or bet_message[0].upper() not in ["T", "X"] or not bet_message[1].isdigit():
          await update.message.reply_text('⚠️ Cú pháp cược không hợp lệ. Vui lòng gửi cược với cú pháp "T [số tiền]" hoặc "X [số tiền]".')
          return
      ...
      if user_info['balance'] < bet_amount:
          remaining_balance = format_currency(user_info['balance'])  
          await update.message.reply_text(f'❌ Bạn chỉ còn {remaining_balance}\n>> không thể cược số tiền lớn hơn số dư của bạn',parse_mode='HTML')
          return
      ...
      if (bet_type == "T" and user_bets["bets"]["X"] > 0) or (bet_type == "X" and user_bets["bets"]["T"] > 0):
          await update.message.reply_text('⚠️ Chỉ đặt cược 1 cửa trong cùng 1 phiên ‼️')
          return
